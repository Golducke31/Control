...
**Alcance movido.** Las sub-rutas de Configuración (`identidad`, `plantillas`, `empresa`, `impuestos`, `integraciones`), las 6 de plataforma y las 3 de Equipo (`miembros`, `roles`, `invitaciones`) quedan como seguimiento; `plataforma/empresas` sí entró porque es donde vive la guarda.

> **Cableadas después de F9.** Las ventanas **Equipo**, **Auditoría** y **Tareas** quedaron al principio con la página declarada y el contrato en `pendiente`, y se cablearon en un commit aparte para que **ninguna de las catorce quede con placeholder**. Tareas trajo además una corrección de modelo: `TareaSchema` —en `gobierno.ts` desde F3— modelaba una tarea de gobierno, con título y vencimiento, que **ninguna ventana usaba**; la ventana Tareas muestra los jobs del motor (`ops.jobs` / `ops.job_runs`). El nombre coincidía con el de la ventana y el contenido no, así que se eliminó el esquema huérfano y se creó `trabajos.ts` con el modelo real, sus dos coherencias (`job_runs_finished_consistency`, `uq_job_runs_one_running`) y el estado derivado —`al_dia`, `atrasado`, `fallando`, `sin_correr`— que es lo que hace útil a la ventana: un job que falla deja un error, uno que dejó de correr no deja nada.
