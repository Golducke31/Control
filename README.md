...
- **Frontend de producción.** **F1 · Fundaciones**, **F2 · Identidad y acceso**, **F3 · Sistema de datos**, **F4 · Operación diaria** y **F5 · Inventario** están hechas: el sistema de diseño en `packages/tokens`, los componentes base en `packages/ui`, `packages/contracts` (esquemas Zod espejo de `pg_enum`, más la lógica pura de transiciones de la cadena de ventas y de inventario), `apps/web` con la carcasa, las catorce ventanas, la sesión (ingreso SSO/credenciales + 2FA, recuperación, invitación, selector de empresa, impersonación), el `ApiClient` con adaptador simulado validado en el borde, la `DataTable` con los cinco estados y los filtros en URL, y las ventanas **Catálogo**, **Panel**, **Ventas** (cadena completa con la barra de acciones gobernada por el estado), **Facturación** (comprobantes con estado de AFIP y de cobro) y **Stock** (niveles, libro mayor, transferencias con bloqueo optimista, recuento y conciliación) cableadas de punta a punta. F5 trae además el primer camino de **escritura** del frontend sobre el adaptador simulado. Faltan F6 a F9 —el contenido de cada ventana— según [`docs/PLAN-FRONTEND-PRODUCCION.md`](docs/PLAN-FRONTEND-PRODUCCION.md), que también lleva el catálogo de ~90 endpoints que el frontend necesita del backend. (Las sub-rutas de detalle de las 15 ventanas de Ventas y las 5 de Facturación quedan como seguimiento dentro de F4; las 5 sub-rutas de Catálogo y `transferencias/nueva` dentro de F5. En los tres casos la base operable y las reglas de su puerta ya están.)
