...
| `npm run verify:accesibilidad` | Una imagen sin `alt`, un control sin nombre accesible, un enlace sin destino, un `tabIndex` positivo, un clic sobre un elemento no interactivo |
| `npm test --workspace @control/web` | Un adaptador HTTP que devuelva algo distinto del simulado, una ruta que el servidor no sirve y el cliente acepte como vacío |
...
- **Frontend de producción. Las nueve fases están implementadas y verificadas.** El sistema de diseño en `packages/tokens` (con las **4 paletas de inquilino** y las **5 plantillas** verificadas contra AA), los componentes base en `packages/ui`, `packages/contracts` (esquemas Zod espejo de `pg_enum`, más la lógica pura de la cadena de ventas, de inventario, del cierre de período y de logística), `apps/web` con la carcasa, las catorce ventanas, la sesión (ingreso SSO/credenciales + 2FA, recuperación, invitación, selector de empresa, impersonación), el `ApiClient` con adaptador simulado validado en el borde, la `DataTable` con los cinco estados y los filtros en URL, las ventanas **Catálogo**, **Panel**, **Ventas**, **Facturación**, **Stock**, **Compras**, **Tesorería**, **Contabilidad** (con **Períodos**), **Fiscal**, **Logística** (con su **torre de control** en vivo) y **Configuración** cableadas de punta a punta, el **tracking público** `/t/[token]`, la **PWA del conductor** `/chofer`, la **consola de plataforma** y el **manejo de errores por ventana** (frontera de error, 404 y frontera global). F5 trajo el primer camino de **escritura**; F6 cerró el hueco del RBAC (**51 permisos en 16 recursos**); F7 el **canal de tiempo real** (una sola conexión SSE por pestaña, multiplexada por tópico); F8 el **lint de textos** (`verify:textos`) con trinquete; y F9 verificó el **criterio 5 de §8.3** —*listo para el backend*— con un servidor de prueba y la equivalencia entre los dos adaptadores, método por método. Lo que queda abierto son cuatro puntos que necesitan navegador (listados en [`docs/PLAN-FRONTEND-PRODUCCION.md`](docs/PLAN-FRONTEND-PRODUCCION.md) §F9): `axe-core` sobre las ventanas, Lighthouse y el gate de presupuestos, los E2E, y la migración completa a `next-intl` —cuya deuda está **medida** en 40 archivos y 187 textos, con un trinquete que impide que crezca—. El plan también lleva el catálogo de ~90 endpoints que el frontend necesita del backend. (Las sub-rutas de detalle de las 15 ventanas de Ventas y las 5 de Facturación quedan como seguimiento dentro de F4; las 5 de Catálogo y `transferencias/nueva` dentro de F5; las 33 de Compras, Tesorería, Contabilidad y Fiscal dentro de F6; las 8 de Logística y `chofer/entrega/[id]` dentro de F7; y las sub-rutas de Configuración, Equipo y plataforma dentro de F8 y F9. En todos los casos la base operable y las reglas de su puerta ya están, y **las catorce ventanas raíz están cableadas de punta a punta**: ninguna muestra un placeholder.)
