 ...
 *
 * Es un componente de **servidor**, y el menú se filtra acá: los permisos y las
 * banderas de funcionalidad se resuelven antes de renderizar, así que una ventana que
 * el usuario no puede ver nunca llega al navegador. Es la regla A2 —el menú se genera
 * desde los permisos, no desde condiciones dispersas en los componentes—.
 *
 * Sólo tres piezas son de cliente, cada una por una razón: la barra lateral porque
 * necesita la ruta activa, el menú móvil porque se despliega, y el selector de empresa
 * porque conserva la ventana al cambiar. El resto —incluido el contenido de cada
 * ventana— se renderiza en el servidor.
 *
 ...
 */
export function Carcasa({ empresa, children }: { empresa: Empresa; children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="sticky top-0 z-30 flex h-[var(--control-alto-encabezado)] shrink-0 items-center gap-2 border-b border-borde-sutil bg-carcasa px-3 sm:px-4">
        ...
        <SelectorDeEmpresa empresas={EMPRESAS} actual={empresa} />
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          */}
          ...
          <span className="flex items-center gap-2 rounded-[var(--control-radio-sm)] px-1.5 py-1">
            ...
            <span className="hidden text-sm text-sobre-carcasa md:block">Emanuel R.</span>
          </span>
        </div>
      </header>
